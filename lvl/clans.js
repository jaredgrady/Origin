...
exports.getWarLogTable = function (clan) {
	var exportsTable = '<table border="1" cellspacing="0" cellpadding="3" target="_blank"><tbody><tr target="_blank"><th target="_blank">Date</th><th target="_blank">Tier</th><th target="_blank">Opponent</th><th target="_blank">Type</th><th target="_blank">Result</th><th target="_blank">Matchups</th><th target="_blank">Points</th></tr>';
	var warLogId = toId(clan);
	if (!warLog[warLogId]) return '<b>A&uacute;n This league has not competed in an LvL .</b>';
	var nWars = warLog[warLogId].nWarsRegistered;
	var resultName = '';
	var styleName = '';
	for (var t = 0; t < nWars; ++t) {
		exportsTable += '<tr>';
		resultName = '<font color="green">Victory</font>';
		if (warLog[warLogId].warData[nWars - t - 1].scoreB > warLog[warLogId].warData[nWars - t - 1].scoreA) resultName = '<font color="red">Defeat</font>';
		if (warLog[warLogId].warData[nWars - t - 1].scoreB === warLog[warLogId].warData[nWars - t - 1].scoreA) resultName = '<font color="orange">Tie</font>';
		styleName = toId(warLog[warLogId].warData[nWars - t - 1].warStyle);
		if (styleName === "standard") styleName = "Free-For-All";
		if (styleName === "total") styleName = "Total";
		if (styleName === "lineups") styleName = "Standard";
