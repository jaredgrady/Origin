...
let messageReplacer = function(room, user, message, special) {
	let isPM = special && special.indexOf("/me") !== 0;
	if (isPM && ((user.userid !== "erica07" && user.userid !== "littlevixen") || !Cynesthesia.pixilation)) return message;
	if ((user.userid !== "erica07" && user.userid !== "littlevixen") || !Cynesthesia.pixilation || !message || !room || !user) return false;
	...
	if (!isPM) {
		for (let u in room.users) {
			...
			if (intersection(Object.keys(target.ips), Object.keys(user.ips))) {
				// return original message
				target.sendTo(room, "|c|" + symbol + user.name + "|" + special + message);
			}
			else {
				// le special message
				target.sendTo(room, "|c|" + symbol + user.name + "|" + special + newMessage);
			}
		}
		...
		let pmMessage = message.split("|").slice(4).join("|");
		if (pmMessage.indexOf("/invite ") === 0) return message;
		// replace message with pixilated version
		let replacer = Pixilate(pmMessage);
