...
exports.commands = {
	...
	tell: function (target, room, user, connection) {
		...
		if (userid.length >= 19) return this.popupReply("\"" + this.targetUsername + "\" is not a legal username.");
			...
			.then(data => {
				nani.get('anime/' + data[0].id)
					.then(data => {
						...
						output += '<tr><td colspan="2" style="' + css + '"><b>Description: </b>' + Tools.escapeHTML(data.description.replace(/(\r\n|\n|\r)/gm, " ")) + '</td></tr>';
						output += '</table></div>';
