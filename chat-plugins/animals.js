...
// IMPORTANT: Check if all images are still valid, if not, replace them.
exports.commands = {
...
/* ADD VIPER IMAGES
	viper: function(target, room, user) {
		if (!this.canBroadcast()) return;
		target = target.toLowerCase();
		let matched = false;
		let image;
...
*/
	...
	animals: function (target, room, user) {
		...
		target = target.toLowerCase();
		if (target === '') {
			return this.sendReplyBox('<center><b>Random Animals</b><br> Use any of the below to generate random animal image:</br><br>puppy - Generates a random dog</br><br>kitten - Generates a random cat</br><br>cow - Generates a random cow</br><br>otter - Generates a random otter</br><br>frog - Generates a random frog</br><br>viper - Generates a random viper</br><br>zoo - Generates a random animal</br><br>-----------------------------------</br><br>Special thanks to Audino Primal, Natsume, tex rex1, Sundar, NTGs, and AuraStormLucario</br></center>');
		}
	},
};
