...
/*	withdraw: function (target, room, user) {
        	if (!user.can('roomleader', null, room) && !room.bankwhitelist.user) return this.sendReply('/withdraw - Access denied.');
        	if (!room.shopBank) return this.sendReply('Please /setbank before using this command.');
        	
        	var amount = isMoney(target);
        	
        	var _this = this;
		Database.read('money', room.shopBank, function (err, balance) {
			if (err) throw err;
			if (!balance) balance = 0;
			if (amount > balance) return _this.sendReply("You cannot withdraw more money than the bank's balance.");
			Database.read('money', user.userid, function (err, userTotal) {
				if (err) throw err
				if (!userTotal) userTotal = 0;
				Database.write('money', balance - amount, room.shopBank, function (err, balance) {
					Database.write('money', userTotal + amount, user.userid, function (err, userTotal) {
						amount = amount + currencyName(amount);
						balance = balance + currencyName(balance);
						userTotal = userTotal + currencyName(userTotal);
						room.add ("|raw|<b>" + user.name + " has withdrawn " + amount + " from the bank:  " + room.shopBank + ".</b>");
						logMoney(user.name + " withdrew " + amount + " from " + room.shopBank + ". " + user.name + " now has " + userTotal + " and " + room.shopBank + " now has " + balance + ".");
					});
				});
			});
		});
	...
	deposit: function (target, room, user) {
        	if (!room.hasShop) return this.sendReply('This room doesn\'t have a league shop.');
        	if (!room.shopBank) room.shopBank = room.founder;
		...
		Database.read('money', user.userid, function (err, userTotal) {
			if (err) throw err;
			if (!userTotal) userTotal = 0;
			if (amount > userTotal) return _this.sendReply("You cannot deposit more money than you have.");
			Database.read('money', room.shopBank, function (err, balance) {
				if (err) throw err
				if (!balance) balance = 0;
				Database.write('money', userTotal - amount, user.userid, function (err, userTotal) {
					Database.write('money', balance + amount, room.shopBank, function (err, balance) {
						...
						balance = balance + currencyName(balance);
						userTotal = userTotal + currencyName(userTotal);
						room.add (user.name + " has deposited " + amount + " to the bank:  " + room.shopBank + ".");
						logMoney(user.name + " deposited " + amount + " from " + room.shopBank + ". " + user.name + " now has " + userTotal + " and " + room.shopBank + " now has " + balance + ".");
					});
				});
			});
		});
