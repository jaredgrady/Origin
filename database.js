...
databases.lowdb = function () {
	...
	methods.remove = function (key, value, username, callback) {
		var user = db('users').find({username: username});
		if (!user) return callback("You don't have this card");
		var obj = {};
		if (!user.hasOwnProperty(key)) {
			return callback("You don't have this card");
		} else {
			objA = user[key];
	...
	methods.findOne = function (key, value, username, callback) {
		var user = db('users').find({username: username});
		if (!user) return callback("You don't have this card");
		var obj = {};
		if (!user.hasOwnProperty(key)) {
