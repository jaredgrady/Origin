...
exports.BattleAbilities = {
	...
	"dreamer": {
		...
		onStart: function (source) {
			if (!source.status) {
				...
				this.add('-message', source.name + "'s Dreamer has sent it into a deep sleep!!");
			}
		},
	},
	"realnigga": {
	...
	"hardbodyintimidation": {
		...
		isNonstandard: true,
		onStart: function (pokemon) {
			this.useMove("hardbodyintimidation", pokemon);
		},
	},
	"indifference": {
		id: "indifference",
	...
	"masterguard": {
		...
		onTryHit: function (pokemon, target, move) {
			if (move.id === 'captivate' || move.id === 'taunt' || move.id === 'disable') {
				this.add('-immune', pokemon, '[msg]', '[from] Master Guard');
				return null;
			}
		},
		onDamage: function (damage, target, source, effect) {
	...
	"spraynpray": {
		...
		onModifyMove: function (move, pokemon, target) {
			if (move.category !== 'Status' && !move.selfdestruct && !move.multihit && ((target.side && target.side.active.length < 2) || move.target in {any:1, normal:1, randomNormal:1})) {
				if (move.id !== 'sexmachineguns' && move.id !== 'firebullet' && move.id !== 'waterbullet' && move.id !== 'grassbullet' && move.id !== 'icebullet' && move.id !== 'darkbullet' && move.id !== 'rockbullet' && move.id !== 'steelbullet' && move.id !== 'ghostbullet' && move.id !== 'psychicbullet' && move.id !== 'dragonbullet' && move.id !== 'fairybullet' && move.id !== 'bugbullet' && move.id !== 'normalbullet' && move.id !== 'fightingbullet' && move.id !== 'flyingbullet' && move.id !== 'poisonbullet' && move.id !== 'electricbullet' && move.id !== 'groundbullet') {
					move.multihit = 5;
					pokemon.addVolatile('spraynpray');
				}
			}
		},
		effect: {
	...
	"graveyard": {
		...
		onStart: function (target, source, effect) {
			this.addPseudoWeather('graveyard', target);
	...
	"shitpostermaximus": {
		id: "shitpostermaximus",
		name: "Shitposter Maximus",
		isNonstandard: true,
		onModifyMove: function (move) {
			if (!move.secondaries) move.secondaries = [];
			if (move.target !== "self") {
	...
	"adaptafilterboost": {
		id: "adaptafilterboost",
		name: "Adaptafilter-boost",
		isNonstandard: true,
		desc: "This Pokemon's moves that match one of its types have a same-type attack bonus (STAB) of 2 instead of 1.5.",
		shortDesc: "This Pokemon's same-type attack bonus (STAB) is 2 instead of 1.5.",
		onStart: function (pokemon) {
	...
	"datassdoe": {
		...
		onAfterDamage: function (damage, target, source, move) {
			if (move && move.flags['contact']) {
				if (this.random(10) < 5) {
					this.boost({accuracy: -2}, source, target);
				}
			}
		},
	},
	...
	"mom": {
		...
		onAfterDamage: function (damage, target, source, move) {
			if (target.hp === 1) {
				this.useMove('healingwish', target);
			}
		},
	},
	"atmosphericdevastation": {
		id: "atmosphericdevastation",
